...
DonutWidget.prototype = {
    ...
    for (var i = 0; i < Object.keys(this.segments).length; i++) {
      ...
      if (!this.$element.find(".donut-bite.large").length) {
        if (s[1] >= this.options.max / 2) {
          this.$element
            .find(".donut-bite[data-segment-index='0']")
            .addClass("large");
        } else {
          this.$element
            .find(".donut-bite[data-segment-index='1']")
            .addClass("large");
        }
      }
  ...
  setStyle: function(start, deg, segment, index) {
    ...
    var selector =
      ".donut-widget[data-chart-instance='" + this.instanceID + "']";
  ...
  removeStyle: function(index) {
    ...
    var split = style.split(
      "/*Style for " + this.instanceID + "_" + index + "*/"
    );
    if (split.length != 1) {
      var part1 = split[0];
      var part2 = split[1].split(
        "/*Style End for " + this.instanceID + "_" + index + "*/"
      )[1];
  ...
  setText: function() {
    if (!this.options.text) {
      this.options.text =
        Math.round((this.options.value / this.options.max) * 100) + "%";
    }
